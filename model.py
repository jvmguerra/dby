...
class DirectoryPage(object):
	...
	def load(table:Table, base:int=0) -> __init__:
		...
		b, n, next = struct.unpack_from(cls.dirFmt, mm, PAGE_SIZE-cls.dirSize)
		...
		file.close()
		return cls(table, b, n, next, entries)

	def save(self):
		cls = DirectoryPage
		file, mm = my_open(self.table.file, self.baseAddr)

		print("tamanho do Directory", len(mm))
...
#todo talvez haverá a necessidade de salvar também a basePage.
	...
	def save(self):
		...
		print("tamanho do RegisterPage", len(mm))
	...
	# checks if there's an unused slot that can hold the register
		for i, s in enumerate(self.slots):
			if s.regSize < 0 and -1*s.regSize >= Reg.size:
